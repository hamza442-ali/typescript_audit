...
const CollateralForm = ({ loanId }: { loanId: number }) => {
  const { supabase, user } = useSupabase();
  const [to, _] = useState(TEMP_NO_PROD_CONTRACT_ADDRESS);
  const [amount, setAmount] = useState();

  const formContext = useForm<CollateralFormProps>({
    defaultValues: {
  ...
  const onSubmit = async (data: CollateralFormProps) => {
    // console.log("Collateral Form Submitted");
    setAmount(amount);
    sendTransaction?.();
  };
  ...
  useEffect(() => {
    const fetchLoans = async () => {
      ...
      const borrower_fee = borrow_amount * 0.03;
      const amount = borrow_amount + borrower_fee;

      formContext.setValue("borrow_amount", borrow_amount.toFixed(2));
      formContext.setValue("borrower_fee", borrower_fee.toFixed(2));
...
const CollateralFormContent = ({ loanId }: { loanId: number }) => {
  ...
  return (
    ...
    >
      <Item sx={{ gridColumn: { xs: "span 4" } }}>
        <Card>
          {/* <CardHeader title={labels.Titles.collateralForm} /> */}
          <CardContent>
            <CollateralForm loanId={loanId} />
...
export default function Page({ params }: BorrowPageProps) {
  ...
  return (
    <ModalSlot title={labels.Titles.collateralForm}>
      <CollateralFormContent loanId={loan_id} />
    </ModalSlot>
  );
}
