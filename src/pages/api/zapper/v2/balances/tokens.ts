...
export async function fetchTokenBalances(params: FetchBalanceParamsOrQuery) {
// We need to first send a POST request to the Zapper API to make sure we are showing the latest balances
  ...
  const response: AxiosResponse<ZapperResultV2BalancesTokensPayload, AxiosError> = await axios
...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
  if (req.method === 'GET') {
    return await fetchTokenBalances(req.query)
